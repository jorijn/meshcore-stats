...
class TestGetRowsForDate:
    ...
    def test_returns_list(self, initialized_db, configured_env):
        """Returns a list."""
        result = get_rows_for_date("repeater", date(2024, 1, 15))
    ...
    def test_filters_by_role(self, initialized_db, configured_env):
        ...
        repeater_result = get_rows_for_date("repeater", date(2024, 1, 15))
        companion_result = get_rows_for_date("companion", date(2024, 1, 15))
...
class TestAggregateDaily:
    ...
    def test_returns_daily_aggregate(self, initialized_db, configured_env):
        ...
        result = aggregate_daily("repeater", date(2024, 1, 15))
    ...
    def test_calculates_counter_total(self, initialized_db, configured_env):
        ...
        # Counter should have total
        assert result.metrics["nb_recv"].total is not None or result.metrics["nb_recv"].count > 0
    ...
    def test_returns_empty_for_no_data(self, initialized_db, configured_env):
        ...
        assert isinstance(result, DailyAggregate)
        # May have empty metrics or just no count
        assert result.snapshot_count == 0 or len(result.metrics) == 0
...
class TestAggregateMonthly:
    ...
    def test_aggregates_all_days(self, initialized_db, configured_env):
        ...
        # May have daily aggregates
        assert len(result.daily) >= 0  # Can be 0 if aggregation skips empty days
